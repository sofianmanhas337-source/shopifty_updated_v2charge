    ...
    try:
        ...
        if card_first_digits in ['4111', '4000']:  # Test cards
            if random.random() < 0.3:  # 30% success rate for test cards
                response = f"""𝘾𝘼𝙍𝘿 ↯ {cc}|{month}|{year}|{cvv}
𝙂𝘼𝙏𝙀𝙒𝘼𝙔 ↯ Stripe + Shopify $9.99 (Graphql)
𝙍𝙀𝙎𝙋𝙊𝙉𝙎𝙀 ↯ Thank you for your purchase! -> 9.99$
𝙏𝙄𝙈𝙀 ↯ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
𝙊𝙬𝙣𝙚𝙧 ↯ @mhitzxg
└─────────────────────┘"""
            else:
                response = f"""𝘾𝘼𝙍𝘿 ↯ {cc}|{month}|{year}|{cvv}
𝙂𝘼𝙏𝙀𝙒𝘼𝙔 ↯ Stripe + Shopify $9.99 (Graphql)
𝙍𝙀𝙎𝙋𝙊𝙉𝙎𝙀 ↯ CARD_DECLINED
𝙏𝙄𝙈𝙀 ↯ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
𝙊𝙬𝙣𝙚𝙧 ↯ @mhitzxg
└─────────────────────┘"""
...
└─────────────────────┘"""
            ...
            if random.random() < 0.15:  # 15% success rate for other cards
                ...
                response = f"""𝘾𝘼𝙍𝘿 ↯ {cc}|{month}|{year}|{cvv}
𝙂𝘼𝙏𝙀𝙒𝘼𝙔 ↯ Stripe + Shopify $9.99 (Graphql)
𝙍𝙀𝙎𝙋𝙊𝙉𝙎𝙀 ↯ {random.choice(success_responses)}
𝙏𝙄𝙈𝙀 ↯ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
𝙊𝙬𝙣𝙚𝙧 ↯ @mhitzxg
└─────────────────────┘"""
            else:
...
⟡ ᴜꜱᴇ /chk ᴛᴏ ᴄʜᴇᴄᴋ ꜱɪɴɢʟᴇ ᴄᴀʀᴅ
⟡ ꜰᴏʀ ᴍᴀꜱꜱ ᴄʜᴇᴄᴋ, ʀᴇᴘʟʏ ᴄᴄ ꜰɪʟᴇ ᴡɪᴛʜ /mchk

ʙᴏᴛ ᴘᴏᴡᴇʀᴇᴅ ʙʏ @mhitzxg""")
    ...
    def process_all():
        ...
        for cc in cc_lines:
            try:
                ...
                if "𝙍𝙀𝙎𝙋𝙊𝙉𝙎𝙀 ↯ Thank you for your purchase!" in result or "𝙍𝙀𝙎𝙋𝙊𝙉𝙎𝙀 ↯ Order Placed!" in result:
                    approved += 1
